...
def save_confusion_matrix(true_labels, pred_labels, label_names, out_path_png, out_path_csv=None):
    """
    true_labels, pred_labels: 1D arrays of ints
    label_names: list of label strings in index order
    """
    ...
    plt.close(fig)
    if out_path_csv:
        np.savetxt(out_path_csv, cm, fmt='%d', delimiter=',')
...
def main():
    ...
    # EVALUATION / PREDICTION -> generate confusion matrix if possible
    if getattr(args, "do_eval", False) or getattr(args, "do_predict", False):
        # choose dataset used for evaluation/prediction in your script
        dataset_to_predict = None
        if 'eval_dataset' in locals():
            dataset_to_predict = eval_dataset
        elif 'test_dataset' in locals():
            dataset_to_predict = test_dataset
        elif 'predict_dataset' in locals():
            dataset_to_predict = predict_dataset

        if dataset_to_predict is None:
            print("No eval/test/predict dataset available for confusion matrix.")
        else:
            pred_out = trainer.predict(dataset_to_predict)
            preds = None
            if pred_out.predictions is not None:
            ...
            if labels is None:
                print("No labels available in dataset; cannot build confusion matrix.")
            else:
                ...
                label_names = None
                if getattr(args, "label_list", None):
                    if isinstance(args.label_list, str):
                        label_names = [l.strip() for l in args.label_list.split(",")]
                    else:
                        label_names = list(args.label_list)
                else:
                    # try model config id2label
                    cfg_id2label = None
                    try:
                        cfg_id2label = trainer.model.config.id2label
                    except Exception:
                        cfg_id2label = None
                    if isinstance(cfg_id2label, dict) and len(cfg_id2label) >= 1:
                        label_names = [cfg_id2label[i] for i in sorted(cfg_id2label.keys())]
                    else:
                        ...
                        if num_labels:
                            ...
                            # fallback
                            unique_labels = sorted(set(labels.tolist() + preds.tolist()))
                            label_names = [str(i) for i in unique_labels]

                out_png = os.path.join(args.output_dir, "confusion_matrix.png")
                out_csv = os.path.join(args.output_dir, "confusion_matrix.csv")
                save_confusion_matrix(labels, preds, label_names, out_png, out_csv)
                print(f"Saved confusion matrix to {out_png} and raw matrix to {out_csv}")
